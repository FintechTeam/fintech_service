...
#         end
        FastImage.size(url_or_file, {custom_header: {"Referer" => "http://mery.jp/search"}})
      end

      def get_html(url_or_file)
        if url_or_file =~ /^https?/
          get_html_by_url(url_or_file, referer: "http://mery.jp/search")
        else
      ...
      def get_html_by_url(url, referer: nil)
        ...
        begin
          ...
          res = conn.get do |req|
            ...
            # senmangaはUserAgentチェック
            req.headers["User-Agent"] = Utils::UserAgent.random
          end
          return res.body
        rescue => e
      ...
      def put_to_file(url, filename)
        html = get_html_by_url(url, referer: "http://mery.jp/search")
        if html.blank?
